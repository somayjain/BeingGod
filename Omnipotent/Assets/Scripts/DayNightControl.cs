...
{
	// Sun and moon UI elements
	public GameObject sun;
	public GameObject moon;
	public float SunMoonRadius = 38;
	...
	void Update()
	{
		...
		if ((currentCycleTime > nightTime || currentCycleTime < dawnTime) && currentPhase == DayPhase.Dusk)
		{
			SetNight();
		}
	...
	{
		worldTimeHour = (int)((Mathf.Round((currentCycleTime / dayCycleLength) * hoursPerDay) + dawnTimeOffset) % hoursPerDay);
	}

	// Updates the sun and moon elements on UI
	private void UpdateSunMoon()
	{
		float moon_angle = (90.0f + (currentCycleTime / dayCycleLength) * 360.0f) * Mathf.PI / 180.0f;
		float sun_angle = moon_angle + Mathf.PI;

		sun.GetComponent<RectTransform> ().anchoredPosition = new Vector2(SunMoonRadius * Mathf.Cos (sun_angle), SunMoonRadius * Mathf.Sin (sun_angle)) ;
		moon.GetComponent<RectTransform> ().anchoredPosition = new Vector2(SunMoonRadius * Mathf.Cos (moon_angle), SunMoonRadius * Mathf.Sin (moon_angle)) ;
	}
